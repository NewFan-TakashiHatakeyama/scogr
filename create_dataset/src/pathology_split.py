...
def create_pathology_train_val_split(base_dir="dataset/Teeth Segmentation on dental X-ray images", 
                                     pathology_labels_dir=None, images_dir=None, val_ratio=0.2):
    """
    Create train-val split for dental pathology annotations.
    ...
    # Find all the pathology label files
    pathology_files = glob.glob(os.path.join(pathology_labels_dir, "*.txt"))
    # Exclude README.md
    pathology_files = [f for f in pathology_files if os.path.basename(f) != "README.md"]
    ...
    train_file = os.path.join(pathology_labels_dir, "train.txt")
    val_file = os.path.join(pathology_labels_dir, "val.txt")
    ...
    with open(train_file, 'w') as f:
        for label_file in train_files:
            ...
            if image_path:
                ...
                rel_path = os.path.relpath(image_path, os.path.dirname(pathology_labels_dir))
                f.write(f"{rel_path}\n")
    ...
    dataset_path = os.path.join(base_dir, "pathology_yolo_dataset")
    ...
    yaml_content = f"""# Dental Pathology Dataset
path: .
train: images/train
val: images/val

nc: 4
names: ["Caries", "Periapical lesion", "Root resorption", "Bone loss"]
"""
    
    yaml_path = os.path.join(dataset_path, "pathology.yaml")
