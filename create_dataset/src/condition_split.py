...
def create_condition_train_val_split(base_dir="dataset/Teeth Segmentation on dental X-ray images", 
                                    condition_labels_dir=None, images_dir=None, val_ratio=0.2):
    """
    Create train-val split for tooth condition annotations.
    ...
    # Find all the condition label files
    condition_files = glob.glob(os.path.join(condition_labels_dir, "*.txt"))
    # Exclude README.md
    condition_files = [f for f in condition_files if os.path.basename(f) != "README.md"]
    ...
    train_file = os.path.join(condition_labels_dir, "train.txt")
    val_file = os.path.join(condition_labels_dir, "val.txt")
    ...
    with open(train_file, 'w') as f:
        for label_file in train_files:
            ...
            if image_path:
                ...
                rel_path = os.path.relpath(image_path, os.path.dirname(condition_labels_dir))
                f.write(f"{rel_path}\n")
    ...
    dataset_path = os.path.join(base_dir, "condition_yolo_dataset")
    ...
    yaml_content = f"""# Dental Condition Dataset
path: .
train: images/train
val: images/val

nc: 3
names: ["Unerupted tooth", "Congenitally missing tooth", "Impacted tooth"]
"""
    
    yaml_path = os.path.join(dataset_path, "condition.yaml")
